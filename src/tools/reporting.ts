...
const assertionParamsSchema = z.object({
  id: z.number().describe('The ID of the assertion from the test assertions list'),
  passed: z.boolean().describe('Whether the assertion passed or failed'),
  evidence: z.string().describe('Specific visual evidence or reason for failure'),
});
...
export const recordAssertionTool = new FunctionTool({
  ...
  execute: async (assertion: any, toolContext) => {
    ...
    const testAssertionsJson = (toolContext.state.get('_test_assertions_json') as string) || '[]';
    ...
    if (record.passed === true) {
      ...
      const presenceIndicators = [
        'is visible', 'are visible', 'is displayed', 'are displayed',
        'is present', 'are present', 'exists', 'exist',
        'is found', 'are found', 'at least one', 'at least',
        'is shown', 'are shown', 'is loaded', 'are loaded',
      ];
      const assertionExpectsPresence = presenceIndicators.some(p => descLower.includes(p));

      const evidenceLower = record.evidence.toLowerCase();
      const absenceIndicators = [
        'not found', 'not visible', 'not present', 'not displayed',
        'does not exist', 'no results', 'was not', 'cannot find',
        'unable to find', 'could not find', 'couldn\'t find',
        'did not find', 'didn\'t find', 'is missing', 'are missing',
        'absent', 'absence', 'nowhere',
        'not on the page', '0 results', 'zero results', 'no matching',
        'not shown', 'does not contain', 'does not have',
        'were not', 'is not', 'are not', 'wasn\'t', 'aren\'t',
        'not located', 'not detected', 'not identified',
      ];
      const evidenceIndicatesAbsence = absenceIndicators.some(p => evidenceLower.includes(p))
        // Also catch "No <X> found/visible/present/..." patterns (e.g., "No Amazon product listings found")
        || /\bno\s+\w+.*?\b(found|visible|present|displayed|shown|exist|appeared|detected)\b/i.test(record.evidence);
    ...
    const unrecordedAssertions = originalAssertions.filter((oa: any) => !recordedIds.has(oa.id));
    ...
    if (unrecordedAssertions.length > 0) {
      message += `. REMAINING: You must still record ${unrecordedAssertions.length} more assertion(s): ${
        unrecordedAssertions.map((a: any) => `ID ${a.id} ("${a.description}")`).join(', ')
      }. Call record_assertion for each one now.`;
    } else {
