...
export async function captureBrowserState(
  ...
  try {
    ...
    if (isVerbose()) {
      log.debug(
        `[capture] Accessibility elements: ${elements.length} (${snapshotLabel})`,
      );
      ...
      try {
        ...
        log.debug(
          `[capture] URL: ${page.url()} | Elements: ${elements.length} | Step: ${stepCount}`,
        );
        ...
        if (elements.length > 0) {
          ...
          log.debug(`[capture] Role breakdown: ${JSON.stringify(roleCounts)}`);
        }
      } catch {
    ...
    log.warn(
      `[capture failed] ${(e instanceof Error ? e : new Error(String(e))).message}. Returning 0 elements to allow loop to continue.`,
    );
    return 0;
  }
}
