 ...
 */
function buildAdvice(result: TestCaseResult, category: FailureCategory): string {
  switch (category) {
 ...
 */
function buildAnalysis(result: TestCaseResult, category: FailureCategory): string {
  const parts: string[] = [];
  ...
  if (category === 'assertion_mismatch' && result.assertions.length > 0) {
    ...
    parts.push(`${failedCount} of ${result.assertions.length} assertions failed.`);
  }

  if (result.validationOutput) {
    const validationSnippet = result.validationOutput.substring(0, 200);
    parts.push(`Validation: ${validationSnippet}${result.validationOutput.length > 200 ? '...' : ''}`);
  }
 ...
 */
function buildFailureReason(result: TestCaseResult, category: FailureCategory): string {
  switch (category) {
    case 'navigation_error':
