    ...
    try {
      ...
      if (isDetached) {
        ...
        try {
          ...
          console.warn(`    \x1b[33m[robustEvaluate]\x1b[0m Got fresh page, URL: ${freshPage.url()}`);
          // Wait for the page to be ready
          await freshPage.waitForSelector('body', { timeout: 5000 }).catch(() => {});
        } catch (refreshErr) {
          console.warn(`    \x1b[33m[robustEvaluate]\x1b[0m Could not refresh page: ${(refreshErr as Error).message}`);
        }
      } else {
        throw e;
      }
