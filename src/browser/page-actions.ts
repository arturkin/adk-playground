...
export async function scrollPage(page: Page, direction: "up" | "down" | "bottom" | "top") {
  const scrollBefore = await page.evaluate(() => ({ x: window.scrollX, y: window.scrollY, pageH: document.body.scrollHeight }));
...
export async function clickElement(page: Page, id: number) {
  ...
  // This fires the full mousedown → mouseup → click event sequence which
  // React/Next.js components need (el.click() only fires a synthetic click
  // that some React components ignore).
