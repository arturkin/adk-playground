...
export async function hoverElement(page: Page, id: number) {
  ...
  const rect = await robustEvaluate(
    page,
    (id) => {
      const el = (window as any).aiElementMap[id];
...
export async function clickElement(page: Page, id: number) {
  ...
  const elInfo = await robustEvaluate(
    page,
    (id) => {
      const el = (window as any).aiElementMap[id];
      if (!el) throw new Error(`Element ${id} not found`);
  ...
  console.log(
    `    \x1b[35m[click_element #${id}]\x1b[0m tag=${elInfo.tag} text="${elInfo.text}" href=${elInfo.href || "none"} class="${elInfo.className}" rect=${JSON.stringify(elInfo.rect)}`,
  );
  ...
  console.log(
    `    \x1b[35m[click_element #${id}]\x1b[0m Mouse click at (${Math.round(elInfo.centerX)}, ${Math.round(elInfo.centerY)})`,
  );
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `    \x1b[33m[click_element #${id}]\x1b[0m Mouse click failed due to detachment/navigation: ${msg}. Assuming navigation started.`,
      );
  ...
  console.log(
    `    \x1b[35m[click_element #${id}]\x1b[0m Current URL: ${currentUrl}`,
  );
}
